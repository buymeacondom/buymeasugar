...
from helpers import (
    parse_proxy_format, test_proxy, test_proxy_detailed, bin_lookup,
...
# ══════════════════════════════════════════════════════════════════════════════
#  CONFIG — all secrets from environment / .env file
# ══════════════════════════════════════════════════════════════════════════════

# Load .env file if present (python-dotenv in requirements.txt)
try:
...
TOKEN = os.environ.get("BOT_TOKEN", "794587178nXs78pFbk0") 

# Bot username WITHOUT the leading @ — used to render command names blue in the
# welcome card (Telegram renders links blue). Set to your real @BotFather username.
BOT_USERNAME = "scarlet"

# ── Join requirements ─────────────────────────────────────────────────────────
join_channel_id = -1004494885271       # Channel ID
join_chat_id    = -1004499669644       # Group ID

CHANNEL_LINK = "https://t.me/+xQAbl2XFPGg2MzRh"
GROUP_LINK   = "https://t.me/+MhuuCYss8fZkNjkx"
...
async def _do_gen_ban(message: types.Message, user_id: int, reason: str, filename: str = "") -> None:
    ...
    file_line = (
        f'\n<tg-emoji emoji-id="5447282724886839705">📁</tg-emoji> '
        f'<b>File</b> ➜ <code>{_html.escape(filename)}</code>'
    ) if filename else ""
    notice = (
        f'<tg-emoji emoji-id="5447647474984449520">🚫</tg-emoji> <b>AUTO-BANNED</b>\n'
        f'━━━━━━━━━━━━━━━━━━━━━━\n'
        f'<tg-emoji emoji-id="5447311106030726740">👤</tg-emoji> <b>User</b> ➜ '
...
E = {
    ...
    "bolt2":     "5224607267797606837",
    "bolt3":     "5276032951342088188",
    "bolt4":     "5222079954421818267",
    "bolt5":     "5341498088408234504",
    "check":     "5206607081334906820",
    "check2":    "5444987348334965906",
    "check3":    "5445210909972655435",
    "cross":     "5210952531676504517",
    "cross2":    "5447647474984449520",
    "cross3":    "5260293700088511294",
    "cross4":    "5411225014148014586",
    "star":      "5438496463044752972",
    "gem":       "5427168083074628963",
    "globe":     "5447410659077661506",
    "link":      "5271604874419647061",
    "chat":      "5443038326535759644",
    "chat2":     "5467538555158943525",
    "link2":     "5222444124698853913",
    "user":      "5447311106030726740",
    "warn":      "5447644880824181073",
    "warn2":     "5274099962655816924",
    "rocket":    "5395695537687123235",
    "sparkle":   "5451882707875276247",
    "hourglass": "5386367538735104399",
    "plus":      "5397916757333654639",
    "dice":      "5361741454685256344",
    "refresh":   "5375338737028841420",
    "bank":      "5444924349754667822",
    "gift":      "5461151367559141950",
    "stop":      "5251203410396458957",
    "loading":   "5447183459602669338",
    "prev":      "5447506720316225765",
    "next":      "5445350109862720603",
    "help_prev": "5447651546613449378",
    "help_next": "5444883062234053429",
    "heart":     "5444887644964159628",
    "star2":     "5325547803936572038",
    "star3":     "5422439311196834318",
    "fire":      "5424972470023104089",
    "fire2":     "5402477260982731644",
    "crown":     "5217822164362739968",
    "crown2":    "5251203410396458957",
    "crown3":    "5391112412445288650",
    "diamond":   "5276032951342088188",
    "coin":      "5447629406057034342",
    "gem2":      "5444860552310457690",
    "gem3":      "5409048419211682843",
    "gem4":      "5233326571099534068",
    "gem5":      "5402186569006210455",
    "lips":      "5395444514028529554",
    "fire3":     "5399913388845322366",
    "spider":    "5321325262674096057",
    "fog":       "5321455971413813888",
    "sparkle2":  "5323572746045654656",
    "fire4":     "5449449325434266744",
    "gem6":      "5447387942995655277",
    "star4":     "5382357040008021292",
    "star5":     "5415655814079723871",
    "heart2":    "5444862970377040012",
    "heart3":    "5445053099989303899",
    "fire5":     "5409109841538994759",
    "gem7":      "5447319442562251569",
    "heart4":    "5444887644964159628",
    "gem8":      "5447453226498552490",
    "star6":     "5444931419270839381",
    "star7":     "5440539497383087970",
    "star8":     "5447203607294265305",
    "gem9":      "5445260044398524944",
    "gem10":     "5447579253723918909",
    "fire6":     "5393512611968995988",
    "fire7":     "5294339927318739359",
    "gem11":     "5447257461889202335",
    "star9":     "5453902265922376865",
    "windchime": "5323388728171850089",
    "fire8":     "5224736245665511429",
    "gem12":     "5229064374403998351",
    "heart5":    "5444862970377040012",
    "gem13":     "5406683434124859552",
    "gem14":     "5458603043203327669",
    "gem15":     "5445030469806618301",
    "ribbon":    "5321128493042396419",
    "gem16":     "5416041192905265756",
    "star_of_david": "5321047313865534150",
    "fire9":     "5416081784641168838",
    "gem17":     "5460755126761312667",
    "fire10":    "5244837092042750681",
    "gem18":     "5463107823946717464",
    "fire11":    "5440660757194744323",
    "star10":    "5397782960512444700",
    "gem19":     "5406756500108501710",
    "star11":    "5391032818111363540",
    "fire12":    "5314504236132747481",
    "heart_black": "5445053099989303899",
    "heart_white": "5444887644964159628",
    "fire13":    "5436113877181941026",
    "couple":    "5445372439397691719",
    "gem20":     "5447642819239903347",
    "gem21":     "5447588260270341594",
    "fire14":    "5460795800101594035",
    "key":       "5445373775132522312",   # placeholder — user will update
    "wrench":    "5341715473882955310",   # placeholder
    "broadcast": "5424818078833715060",   # placeholder
}
...
R = {
    "cc":         "5447453226498552490",
    "gate":       "5444869180899752137",
    "price":      "5409048419211682843",
    "bin_info":   "5231200819986047254",
    "visa":       "5447453226498552490",
    "master":     "5447453226498552490",
    "amex":       "5447453226498552490",
    "type":       "5445146408153806223",
    "level":      "5444931419270839381",
    "bank":       "5444924349754667822",
    "country":    "5447410659077661506",
    "checked_by": "5447311106030726740",
}
...
_BOLD_MAP = {}
# Uppercase A-Z → 𝑨-𝒁 (U+1D468 to U+1D487) Mathematical Bold Italic
for i, c in enumerate("ABCDEFGHIJKLMNOPQRSTUVWXYZ"):
    _BOLD_MAP[c] = chr(0x1D468 + i)
# Lowercase a-z → 𝒂-𝒛 (U+1D482 to U+1D4A1) Mathematical Bold Italic
for i, c in enumerate("abcdefghijklmnopqrstuvwxyz"):
    _BOLD_MAP[c] = chr(0x1D482 + i)
# Digits 0-9 → 𝟎-𝟵 (U+1D7CE to U+1D7D7) Mathematical Bold
...
def _mbi(text: str) -> str:
    """Convert EVERY ASCII letter/digit to Mathematical Bold Italic.

    Applied at the send boundary so no outgoing message text is left in plain
    ASCII.  Skips HTML tags (<...>), HTML entities (&amp;), and bare URLs
    (kept plain so Telegram still auto-links them).  Idempotent — text already
    converted (via bold()/_to_mi()) passes through unchanged.
    """
    if not text:
        return text
    out = []
    ...
    while i < n:
        ...
        if c == '<':
            ...
            if m:
                ...
                out.append('&')
                i += 1
        elif c in ('h', 'H', 'w', 'W') and (text[i:i + 4].lower() == 'http' or text[i:i + 3].lower() == 'www'):
...
def _elve_gold_card(
    ...
    """Build an ELVE GOLD style result card.
    ...
    Layout (per spec):
        ...
        ⚡ Code → ORDER_PLACED
        ━━━━━━━━━━━━
        BIN / Type / Level / Bank / Country block
        ━━━━━━━━━━━━
        User → @username
        Powered by @goonermen
    """
    lines = [status_line, _SEP]
    lines.append(f"{pe(R['cc'])} {bold('Card')} → <tg-spoiler>{card_field}</tg-spoiler>")
    lines.append(f"{pe(R['gate'])} {bold('Gate')} → {bold(gate_label)}")
    if site:
        lines.append(f"{pe(E['globe'])} {bold('Site')} → {bold(site)}")
    if amount and str(amount) not in ("-", "0", "None"):
        lines.append(f"{pe(R['price'])} {bold('Amount')} → {bold(str(amount))}")
    if response_code:
        lines.append(f"{pe(E['bolt'])} {bold('Code')} → {bold(response_code)}")
    lines.append(_SEP)
    if bin_info:
        b = bin_info
        lines.append(f"{brand_emoji(b.get('brand', ''))}{bold('BIN')} → {bold(b.get('brand', '-'))}")
        lines.append(f"{pe(R['type'])} {bold('Type')} → {bold(b.get('type', '-'))}")
        lines.append(f"{pe(R['level'])} {bold('Level')} → {bold(b.get('level', '-'))}")
        lines.append(f"{pe(R['bank'])} {bold('Bank')} → {bold(b.get('bank', '-'))}")
        lines.append(f"{pe(R['country'])} {bold('Country')} → {b.get('flag', '')} {bold(b.get('country', '-'))}")
    lines.append(_SEP)
    if user_display:
        lines.append(f"{pe(R['checked_by'])} {bold('User')} → {user_display}")
    lines.append(f"{pe(E['sparkle'])} {bold('Powered by @goonermen')}")
    return "\n".join(lines)


def _mask_site(site: str, max_len: int = 50) -> str:
    """Mask the middle of a site subdomain (ELVE GOLD spec style).

    'ymcaXXXXXXXXXXXXXXXXXXds.myshopify.com' → 'ymca****...****ds.myshopify.com'
    """
    s = site.split("//")[1] if "//" in site else site
    if "." in s:
        sub, _, rest = s.partition(".")
        if len(sub) > 6:
            s = sub[:4] + "*" * (len(sub) - 6) + sub[-2:] + "." + rest
    return s[:max_len]


def _progress_bar(done: int, total: int, width: int = 18) -> str:
    """Render an ELVE GOLD style progress bar: ████████░░░░░░░░ 50.0%"""
    if total <= 0:
        filled = width
        pct = 100.0
    else:
...
def _fmt_elapsed(seconds: float) -> str:
    """Format elapsed seconds as HH:MM:SS (zero-padded, per ELVE GOLD spec)."""
...
def _elve_gold_progress(
    title: str,
    done: int, total: int,
    charged: int = 0, approved: int = 0, declined: int = 0, errors: int = 0,
    elapsed: float = 0.0,
    extra: str = "",
) -> str:
    """Build an ELVE GOLD style file-check progress card (per spec)."""
    lines = [
        f"{pe(E['rocket'])} {bold(title)}",
        _SEP,
        _progress_bar(done, total),
        f"{pe(E['bolt'])} {bold('Total')} → {bold(str(total))}",
        f"{pe(E['gem'])} {bold('Checked')} → {bold(str(done))}",
        ...
        _SEP,
        f"{pe(E['hourglass'])} {bold('Elapsed')} → {bold(_fmt_elapsed(elapsed))}",
    ]
    if extra:
        lines.append(extra)
    return "\n".join(lines)
...
# ══════════════════════════════════════════════════════════════════════════════
#  MATH BOLD ITALIC EVERYWHERE — transform every outgoing message at the send
#  boundary so no user-facing text is ever left in plain ASCII.  Idempotent:
#  text already converted via bold()/_to_mi()/_mbi passes through unchanged.
# ══════════════════════════════════════════════════════════════════════════════

_orig_send_message = bot.send_message


async def _mbi_send_message(chat_id, text=None, *args, **kwargs):
    if text is not None:
        text = _mbi(str(text))
...
async def _mbi_edit_text(text=None, *args, **kwargs):
    if text is not None:
        text = _mbi(str(text))
    return await _orig_edit_text(text, *args, **kwargs)


def _wrap_caption_method(orig):
    async def _wrapper(*args, **kwargs):
        if kwargs.get("caption") is not None:
            kwargs["caption"] = _mbi(str(kwargs["caption"]))
        return await orig(*args, **kwargs)
    return _wrapper


bot.send_message = _mbi_send_message
bot.edit_message_text = _mbi_edit_text
for _meth in ("send_photo", "send_document", "send_audio", "send_video",
              "send_animation", "send_sticker", "edit_message_caption"):
    _orig = getattr(bot, _meth, None)
    if _orig is not None:
        setattr(bot, _meth, _wrap_caption_method(_orig))

_orig_answer_cb = bot.answer_callback_query


async def _mbi_answer_cb(callback_query_id, text=None, *args, **kwargs):
    if text is not None:
        text = _mbi(str(text))
    return await _orig_answer_cb(callback_query_id, text, *args, **kwargs)


bot.answer_callback_query = _mbi_answer_cb
    ...
    return {
        "inline_keyboard": [
            ...
            [{
                ...
                "icon_custom_emoji_id": "5447311106030726740",
                "style": "primary"
            }],
        ]
    }
...
# ── Premium emoji placeholders for the /start welcome card ──
# Replace the IDs below with your actual premium emoji IDs from @K11R7A / @goonermen.
_WC = {
    "face":   "5461117441612462242",  # brand smiley (placeholder — reuse "plus")
    "limit":  "5447453226498552490",  # CC Limit icon (reuse R['cc'])
    "access": "5240241223632954241",  # Access X icon (reuse E['cross'])
    "cmd_sc":     "5456140674028019486",  # /sc
    "cmd_msc":    "5445059250382469069",  # /msc
    "cmd_msctxt": "5447389837076231920",  # /msctxt
    "cmd_hit":    "5424972470023104089",  # /hit (reuse bolt2)
    "cmd_proxy":  "5271604874419647061",  # /proxy
    "cmd_myproxy":"5231012545799666522",  # /myproxy
    "cmd_rmproxy":"5210952531676504517",  # /rmproxy
    "cmd_bin":    "5444989577422993015",  # /bin
    "cmd_redeem": "5445373775132522312",  # /redeem
    "cmd_me":     "5447311106030726740",  # /me
}


def _welcome_card(user) -> str:
    """X D Bot style welcome/profile card — matches the /start screenshot.

    Layout (Mathematical Bold Italic throughout):
        {face} scarlet

        Welcome, @username scarlet
        ...
        ▸ Commands
        💡 /sc — single card check
        ...
    Command names render blue (Telegram link colour).
    All text uses Mathematical Bold Italic (via bold()).
    """
    uid = user.id
    ...
    if username:
        display = f"@{username}"
        link = f"https://t.me/{_html.escape(username)}"
    else:
        display = full_name or str(uid)
    ...
    return (
        ...
        f"{bold('Welcome')}, <a href=\"{link}\">{_html.escape(display)}</a> {bold('scarlet')}\n\n"
        f"│ {bold('Role')} → {bold(role.capitalize())}\n"
        f"{pe(_WC['limit'])} {bold('CC Limit')} → {bold(str(cc_limit))}\n"
        f"{pe(_WC['access'])} {bold('Access')} → {bold(access)}\n\n"
        f"▸ {bold('Commands')}\n"
        f"{pe(_WC['cmd_sc'])} {_c('/sc')} — {bold('single card check')}\n"
        f"{pe(_WC['cmd_msc'])} {_c('/msc')} — {bold('mass check (inline)')}\n"
        f"{pe(_WC['cmd_msctxt'])} {_c('/msctxt')} — {bold('mass check (.txt file)')}\n"
        f"{pe(_WC['cmd_hit'])} {_c('/hit')} — {bold('stripe checkout')}\n"
        f"{pe(_WC['cmd_proxy'])} {_c('/proxy')} — {bold('set your proxy (inline or reply to .txt)')}\n"
        f"{pe(_WC['cmd_myproxy'])} {_c('/myproxy')} — {bold('view current proxy')}\n"
        f"{pe(_WC['cmd_rmproxy'])} {_c('/rmproxy')} — {bold('remove proxy')}\n"
        f"{pe(_WC['cmd_bin'])} {_c('/bin')} — {bold('BIN lookup')}\n"
        f"{pe(_WC['cmd_redeem'])} {_c('/redeem')} — {bold('redeem a key')}\n"
        f"{pe(_WC['cmd_me'])} {_c('/me')} — {bold('your profile')}"
    )
...
async def cb_verify_join(callback: types.CallbackQuery):
    ...
    await safe_edit(callback.message, _welcome_card(callback.from_user), reply_markup=menu_keyboard())
...
async def cb_menu_back(callback: types.CallbackQuery):
    ...
    try:
        await safe_edit(callback.message, 
            _welcome_card(callback.from_user),
            reply_markup=menu_keyboard(),
        )
    except Exception:
        pass
...
async def cmd_proxy(message: types.Message):
    ...
    for batch_start in range(0, len(parsed_list), TEST_BATCH):
        ...
        async def _test_one(proxy_data):
            try:
                # Use detailed test for better diagnostics
                result = await test_proxy_detailed(proxy_data["proxy_url"])
                proxy_data["_test_result"] = result
                return proxy_data if result["success"] else None
            except Exception as e:
                proxy_data["_test_result"] = {"success": False, "error": str(e)[:100], "endpoint": None, "all_results": []}
                return None
        ...
        for r in results:
            if r is not None and len(working) < need:
                working.append(r)
                detailed_results.append(r["_test_result"])
            elif r is None:
                dead += 1
    ...
    if not working:
        await safe_edit(status_msg, 
            ...
            f"{pe(E['bolt'])} {bold('Tip: Try different proxy types (HTTP/SOCKS5) or check connectivity.')}"
        )
        return
    ...
    if endpoint_stats:
        result_lines.append(f"\n{pe(E['globe'])} {bold('Successful endpoints:')}")
        for ep, count in sorted(endpoint_stats.items(), key=lambda x: -x[1]):
            result_lines.append(f"  {pe(E['next'])} {ep} → {count} proxies")
...
async def cb_check_proxy(callback: types.CallbackQuery):
    ...
    detailed_results = []
    for p in proxy_list:
        if "_test_result" in p:
            detailed_results.append(p["_test_result"])
...
async def cmd_sh(message: types.Message):
    ...
    if not cc_str:
        await message.reply(
            ...
            f"{pe(E['next'])} {bold('Usage:')} /sc 4388540109154632|03|2030|815\n"
    ...
    result_text = _elve_gold_card(
        status_line=status_line,
        card_field=cc_str,
        gate_label=str(gate),
        amount=str(price),
        response_code=str(response)[:60],
        site=_mask_site(site),
        bin_info=bin_info,
        user_display=user_link(message.from_user.id, message.from_user.full_name, message.from_user.username),
    )
...
async def _send_charged_notification(
    ...
    try:
        ...
        _CE_HDR    = pe(random.choice(["5427168083074628963","5438496463044752972","5325547803936572038"])
                        if _is_order else
                        random.choice(["5447644880824181073","5274099962655816924","5440660757194744323"]))
        _CE_UNAME  = pe(random.choice(["5447311106030726740","5447311106030726740","5337080053119336309"]))
        _CE_NAME   = pe(random.choice(["5447311106030726740","5445224894386172410","5447226864542190055"]))
        _CE_STATUS = pe(random.choice(["5427168083074628963","5206607081334906820","5341498088408234504"])
                        if _is_order else
                        random.choice(["5447644880824181073","5274099962655816924","5440660757194744323"]))
        _CE_PRICE  = pe(random.choice(["5409048419211682843","5233326571099534068","5231005931550030290"]))
        _CE_GATE   = pe(random.choice(["5444869180899752137","5341715473882955310","5424818078833715060"]))
...
@router.message(Command("msc", "msh"))
...
# ══════════════════════════════════════════════════════════════════════════════
#  /adc COMMAND — Admin/Owner Command List (visible to admin+ only)
# ══════════════════════════════════════════════════════════════════════════════

@router.message(Command("adc"))
async def cmd_adc(message: types.Message):
    """Show admin/owner commands — only visible to admins and owner."""
    if not auth.is_admin(message.from_user.id):
        await message.reply(f"{pe(E['cross'])} {bold('Admin only command!')}")
        return

    text = (
        f"{pe(E['crown'])} {bold('Admin & Owner Commands')}\n\n"
        f"{pe(E['bolt'])} /admin {bold('user-id')} — {bold('Add admin (owner only)')}\n"
        f"{pe(E['bolt'])} /unadmin {bold('user-id')} — {bold('Remove admin (owner only)')}\n\n"
        f"{pe(E['gem'])} /auth {bold('user-id [days]')} — {bold('Grant premium (admin+)')}\n"
        f"{pe(E['cross'])} /unauth {bold('user-id')} — {bold('Remove premium (admin+)')}\n"
        f"{pe(E['cross'])} /ban {bold('user-id')} — {bold('Ban user (admin+)')}\n"
        f"{pe(E['check'])} /unban {bold('user-id')} — {bold('Unban user (admin+)')}\n"
        f"{pe(E['key'])} /key {bold('users days [credits]')} — {bold('Generate key (admin+)')}\n"
        f"{pe(E['rocket'])} /ckey {bold('users credits')} — {bold('Credits-only key (admin+)')}\n"
        f"{pe(E['gem'])} /pkey {bold('users days credits')} — {bold('Premium+credits key (admin+)')}\n\n"
        f"{pe(E['globe'])} /filter {bold('site')} — {bold('Filter sites (owner only)')}\n"
        f"{pe(E['wrench'])} /imaintain {bold('reason')} — {bold('Enable maintenance (owner only)')}\n"
        f"{pe(E['check'])} /cmaintain — {bold('Disable maintenance (owner only)')}\n"
        f"{pe(E['globe'])} /api — {bold('API status (owner only)')}\n"
        f"{pe(E['key'])} /nopecha {bold('api_key')} — {bold('Set captcha key (owner only)')}\n"
        f"{pe(E['broadcast'])} /broad {bold('message')} — {bold('Broadcast (admin+)')}\n\n"
        f"{pe(E['star'])} {bold('Site Management:')}\n"
        f"{pe(E['plus'])} /stsite {bold('url')} — {bold('Add ST site (owner)')}\n"
        f"{pe(E['cross'])} /strem {bold('#')} — {bold('Remove ST site (owner)')}\n"
        f"{pe(E['link'])} /stmysite — {bold('View ST sites (owner)')}\n"
        f"{pe(E['plus'])} /rzsite {bold('url')} — {bold('Add RZ site (owner)')}\n"
        f"{pe(E['cross'])} /rzrem {bold('#')} — {bold('Remove RZ site (owner)')}\n"
        f"{pe(E['link'])} /rzmysite — {bold('View RZ sites (owner)')}\n\n"
        f"{pe(E['sparkle'])} {bold('Use')} /cmds {bold('for user commands.')}"
    )
...
async def cb_quick_check(callback: types.CallbackQuery):
    ...
    result_text = _elve_gold_card(
        ...
        amount=str(price),
        response_code=str(response)[:60],
        site=_mask_site(site) if site else "",
        bin_info=bin_info,
        user_display=user_link(callback.from_user.id, callback.from_user.full_name, callback.from_user.username),
    )
...
# Responses that indicate a failed/errored check (counted under "Errors").
_ERR_INDICATORS = (
    'error', 'timeout', 'exception', 'no response', 'connection',
    'refused', 'unreachable', 'failed to', 'bad gateway',
)
...
async def _deliver_ran_hit(
    ...
    try:
        ...
        hit_text = _elve_gold_card(
            ...
            bin_info=bin_info,
            user_display=user_link(user_id, user_name, user_uname),
        )
...
async def _process_ran_cards(
    ...
    async def _maybe_update_status(force: bool = False):
        ...
        progress_text = _elve_gold_progress(
            title="Live Checking",
            done=_done, total=total,
            charged=charged, approved=approved, declined=declined,
            errors=errors,
    ...
    async def worker():
        ...
        while not _RAN_STOP_FLAGS.get(stop_key):
            ...
            is_error = any(ind in response for ind in _ERR_INDICATORS)
...
@router.message(Command("msctxt", "ran"))
...
async def cmd_auth(message: types.Message):
    ...
    try:
        await bot.send_message(
            ...
            f"{pe(E['next'])} /sc — {bold('Check CC')}\n"
            f"{pe(E['next'])} /msc — {bold('Mass Check')}\n"
            f"{pe(E['next'])} /msctxt — {bold('File Check')}\n\n"
...
# ════════════════════════════════════════════════════════════════════════════════
#  /ckey COMMAND — Admin generates credits-only key
# ════════════════════════════════════════════════════════════════════════════════
...
async def cmd_ckey(message: types.Message):
    ...
    if len(args) < 3 or not args[1].isdigit() or not args[2].isdigit():
        await message.reply(
            f"{pe(E['warn'])} {bold('Usage:')} /ckey {bold('users')} {bold('credits')}\n\n"
            f"{pe(E['next'])} {bold('Example:')} /ckey 50 5000\n"
            f"{pe(E['next'])} {bold('Generates 1 key — 50 users can redeem, 5000 credits each')}"
        )
        return
    ...
    if credits < 1 or credits > 100000:
        await message.reply(f"{pe(E['cross'])} {bold('Credits must be 1-100000')}")
        return

    key = auth.generate_key("ckey", max_users, credits=credits)

    text = (
        f"{pe(E['rocket'])} {bold('Credits Key Generated')} {pe(E['check'])}\n"
        f"━━━━━━━━━━━━━━━━━━━━\n\n"
        f"┣ {pe(E['bolt'])} {bold('Key')} ➜ <code>{key}</code>\n"
        f"┣ {pe(E['user'])} {bold('Slots')} ➜ {bold(str(max_users))} {bold('users can redeem')}\n"
        f"┣ {pe(E['star'])} {bold('Credits')} ➜ {bold(str(credits))} {bold('credits each')}\n\n"
        f"{pe(E['sparkle'])} {bold('Users redeem with')} /redeem {key} {pe(E['bolt'])}"
    )
    await message.reply(text)


# ════════════════════════════════════════════════════════════════════════════════
#  /pkey COMMAND — Admin generates premium + credits key
# ════════════════════════════════════════════════════════════════════════════════
...
async def cmd_pkey(message: types.Message):
    ...
    if len(args) < 4 or not args[1].isdigit() or not args[2].isdigit() or not args[3].isdigit():
        await message.reply(
            f"{pe(E['warn'])} {bold('Usage:')} /pkey {bold('users')} {bold('days')} {bold('credits')}\n\n"
            f"{pe(E['next'])} {bold('Example:')} /pkey 10 30 1000\n"
            f"{pe(E['next'])} {bold('Generates 1 key — 10 users, 30 days premium + 1000 credits each')}"
        )
        return
    ...
    if days < 0 or days > 3650:
        await message.reply(f"{pe(E['cross'])} {bold('Days must be 0-3650 (0 = lifetime)')}")
        return
    if credits < 0 or credits > 100000:
        await message.reply(f"{pe(E['cross'])} {bold('Credits must be 0-100000')}")
        return

    key = auth.generate_key("pkey", max_users, days=days, credits=credits)

    plan_text = f"{days} days" if days > 0 else "lifetime"
    text = (
        f"{pe(E['gem'])} {bold('Premium + Credits Key Generated')} {pe(E['check'])}\n"
        f"━━━━━━━━━━━━━━━━━━━━\n\n"
        f"┣ {pe(E['bolt'])} {bold('Key')} ➜ <code>{key}</code>\n"
        f"┣ {pe(E['user'])} {bold('Slots')} ➜ {bold(str(max_users))} {bold('users can redeem')}\n"
        f"┣ {pe(E['star'])} {bold('Plan')} ➜ {bold(plan_text)} {bold('premium')}\n"
        f"┣ {pe(E['star'])} {bold('Credits')} ➜ {bold(str(credits))} {bold('credits each')}\n\n"
        f"{pe(E['sparkle'])} {bold('Users redeem with')} /redeem {key} {pe(E['bolt'])}"
    )
    await message.reply(text)
...
async def cmd_redeem(message: types.Message):
    ...
    if len(args) < 2 or not args[1].strip():
        await message.reply(
            f"{pe(E['warn'])} {bold('Usage:')} /redeem {bold('goon-xxxxxxxx')}"
        )
        return
...
def _ayd_cc_block(cc: str, result: dict) -> str:
    ...
    return (
        f"{sl}\n"
        f"{pe(R['cc'])} {bold('Card')} → <tg-spoiler>{cc}</tg-spoiler>\n"
        f"{pe(R['gate'])} {bold('Code')} → {bold(raw)}"
        f"{tds_line}"
    )
...
async def cmd_ayd(message: types.Message):
    ...
    if ayden is None:
        await message.reply(
            f"{pe(E['cross'])} {bold('Adyen module not available')}\n\n"
            f"{pe(E['warn'])} {bold('Adyen gateway file is missing on this server.')}"
        )
        return
...
async def cmd_st(message: types.Message):
    ...
    result_text = _elve_gold_card(
        status_line=sl,
        card_field=cc_str,
        gate_label="WooCommerce Stripe",
        amount="-",
        response_code=str(result_str)[:120],
        site=_mask_site(site_url) if site_url else "",
        bin_info=bin_info,
        user_display=user_link(message.from_user.id, message.from_user.full_name, message.from_user.username),
    )
    ...
    try:
        for i in range(0, len(all_ccs), STXT_BATCH_SIZE):
            ...
            if _now - _last_status_edit >= 2.5 or checked >= total:
                ...
                try:
                    progress_text = _elve_gold_progress(
                        title="ST Live Checking",
                        done=checked, total=total,
                        charged=0, approved=approved, declined=declined,
                        errors=0, elapsed=elapsed,
                        extra=f"{pe(R['checked_by'])} {bold('User')} → {user_link(user_id, user_name, user_uname)}",
                    )
                    await safe_edit(status_msg, progress_text,
                        reply_markup={
                            "inline_keyboard": [[{
...
# ════════════════════════════════════════════════════════════════════════════════
#  GATEWAY site storage (gatesite.json) — per-user custom gate sites
# ════════════════════════════════════════════════════════════════════════════════

_gatesite_cache: dict | None = None
_gatesite_cache_mtime: float = 0.0
GATESITE_MAX = 25  # max gate sites per user per gate type
GATE_TYPES = ["stripe", "razorpay", "braintree", "shopify", "woocommerce", "adyen", "paypal", "square", "authorize", "other"]


def _load_gatesites() -> dict:
    global _gatesite_cache, _gatesite_cache_mtime
    try:
        mt = os.path.getmtime(GATESITE_FILE)
    except OSError:
        return {}
    if _gatesite_cache is not None and mt == _gatesite_cache_mtime:
        return _gatesite_cache
    try:
        with open(GATESITE_FILE, "r", encoding="utf-8") as f:
            _gatesite_cache = json.load(f)
            _gatesite_cache_mtime = mt
            return _gatesite_cache
    except Exception:
        return {}


def _save_gatesites(data: dict):
    global _gatesite_cache, _gatesite_cache_mtime
    with open(GATESITE_FILE, "w", encoding="utf-8") as f:
        json.dump(data, f, indent=2, ensure_ascii=False)
    _gatesite_cache = data
    try:
        _gatesite_cache_mtime = os.path.getmtime(GATESITE_FILE)
    except OSError:
        _gatesite_cache_mtime = 0.0


def _normalize_gatesite_url(url: str) -> str:
    return url.strip().rstrip("/")


def _coerce_gatesite_list(val) -> list[str]:
    if isinstance(val, list):
        out = []
        for v in val:
            if isinstance(v, dict):
                v = v.get("url") or v.get("site") or ""
            if v and isinstance(v, str):
                out.append(_normalize_gatesite_url(v))
        return out
    if isinstance(val, dict):
        v = val.get("url") or val.get("site") or ""
        return [_normalize_gatesite_url(v)] if v else []
    if isinstance(val, str) and val:
        return [_normalize_gatesite_url(val)]
    return []


def get_user_gatesites(user_id: int, gate_type: str) -> list[str]:
    """Return all saved gate sites for a user and gate type."""
    data = _load_gatesites()
    user_data = data.get(str(user_id), {})
    return _coerce_gatesite_list(user_data.get(gate_type, []))


def get_user_gatesite(user_id: int, gate_type: str) -> str | None:
    """Return a random gate site from the user's list for a gate type."""
    sites = get_user_gatesites(user_id, gate_type)
    return random.choice(sites) if sites else None


def add_user_gatesite(user_id: int, gate_type: str, url: str) -> str:
    """Add a gate site for a specific gate type. Returns 'added', 'duplicate', or 'limit'."""
    if gate_type not in GATE_TYPES:
        return "invalid_gate_type"
    url = _normalize_gatesite_url(url)
    data = _load_gatesites()
    user_data = data.get(str(user_id), {})
    sites = _coerce_gatesite_list(user_data.get(gate_type, []))
    if url in sites:
        return "duplicate"
    if len(sites) >= GATESITE_MAX:
        return "limit"
    sites.append(url)
    user_data[gate_type] = sites
    data[str(user_id)] = user_data
    _save_gatesites(data)
    return "added"


def del_user_gatesite(user_id: int, gate_type: str, index: int | None = None):
    """Remove gate site by 1-based index, or all sites for gate_type if index is None."""
    data = _load_gatesites()
    key = str(user_id)
    user_data = data.get(key, {})
    sites = _coerce_gatesite_list(user_data.get(gate_type, []))
    if index is None:
        user_data.pop(gate_type, None)
        if not user_data:
            data.pop(key, None)
    else:
        if 1 <= index <= len(sites):
...
def get_all_user_gatesites(user_id: int) -> dict:
    """Return all gate sites for all gate types for a user."""
    data = _load_gatesites()
    user_data = data.get(str(user_id), {})
    result = {}
    for gate_type in GATE_TYPES:
        sites = _coerce_gatesite_list(user_data.get(gate_type, []))
        if sites:
            result[gate_type] = sites
    return result


# Gate type detection keywords for auto-detection
GATE_TYPE_KEYWORDS = {
    "stripe": ["stripe", "checkout.stripe", "pay.stripe"],
    "razorpay": ["razorpay", "razorpay.me", "pages.razorpay"],
    "braintree": ["braintree", "btree"],
    "shopify": ["myshopify", "checkout.shopify", "shopify"],
    "woocommerce": ["woocommerce", "wc-ajax", "wc_checkout"],
    "adyen": ["adyen", "checkout.adyen"],
...
def detect_gate_type(url: str) -> str:
    """Auto-detect gate type from URL."""
    url_lower = url.lower()
    for gate_type, keywords in GATE_TYPE_KEYWORDS.items():
        for kw in keywords:
            if kw in url_lower:
                return gate_type
    return "other"
        ...
        for i in range(0, len(all_ccs), RZTXT_BATCH_SIZE):
            ...
            for fut in asyncio.as_completed(wrapped):
                ...
                if _now - _last_status_edit >= 3 or (checked + skipped) >= total:
                    ...
                    progress_text = _elve_gold_progress(
                        title="RZ Live Checking",
                        done=checked + skipped, total=total,
                        charged=charged_count, approved=approved, declined=declined,
                        errors=0, elapsed=time.time() - _start_time,
                        extra=(
...
async def cmd_chk(message: types.Message):
    ...
    result_text = _elve_gold_card(
        status_line=sl,
        card_field=cc_str,
        gate_label="Stripe Auth",
        amount="-",
        response_code=str(msg)[:120],
        bin_info=bin_info,
        user_display=user_link(message.from_user.id, message.from_user.full_name, message.from_user.username),
    )
    ...
    try:
        for i in range(0, len(all_ccs), CHKTXT_BATCH):
            ...
            if _now - _last_status_edit >= 2.5 or checked >= total:
                ...
                try:
                    progress_text = _elve_gold_progress(
                        title="Stripe Auth Live Checking",
                        done=checked, total=total,
                        charged=0, approved=approved, declined=declined,
                        errors=0, elapsed=elapsed,
                        extra=f"{pe(R['checked_by'])} {bold('User')} → {user_link(user_id, user_name, user_uname)}",
...
def _vbv_result_text(
    ...
    card_type = "2D" if code == "passed" else ("3D" if code == "challenge_3d" else "-")
    code_field = f"{card_type}: {message}" if card_type != "-" else str(message)
    return _elve_gold_card(
        status_line=sl,
        card_field=cc_str,
        gate_label="Braintree VBV",
        amount="-",
        response_code=code_field,
        bin_info=bin_info,
        user_display=user_link(user_id, user_name, user_uname),
    )
...
def _usd1_result_text(
    ...
    return _elve_gold_card(
        ...
        gate_label=str(gate),
        amount="$1",
        response_code=_gate_msg_display(msg),
        bin_info=bin_info,
        user_display=user_link(user_id, user_name, user_uname),
    )
        ...
        for i in range(0, len(all_ccs), USD1TXT_BATCH_SIZE):
            ...
            for fut in asyncio.as_completed(wrapped):
                ...
                if _now - _last_status_edit >= 3 or (checked + skipped) >= total:
                    ...
                    progress_text = _elve_gold_progress(
                        title=f"{gate_label} Live Checking",
                        done=checked + skipped, total=total,
                        charged=charged_count, approved=approved, declined=declined,
                        errors=0, elapsed=time.time() - _start_time,
                        extra=(
...
def _skcvv_result_text(
    ...
    return _elve_gold_card(
        status_line=sl,
        card_field=cc_str,
        gate_label=str(SKCVV_GATE_LABEL),
        ...
        for i in range(0, len(all_ccs), SKTXT_BATCH_SIZE):
            ...
            for fut in asyncio.as_completed(wrapped):
                ...
                if _now - _last_status_edit >= 3 or (checked + skipped) >= total:
                    ...
                    progress_text = _elve_gold_progress(
                        title=f"{SKCVV_GATE_LABEL} Live Checking",
                        done=checked + skipped, total=total,
                        charged=charged_count, approved=approved, declined=declined,
                        errors=0, elapsed=time.time() - _start_time,
                        extra=(
        ...
        for i in range(0, total, BRTXT_BATCH):
            ...
            for fut in asyncio.as_completed(wrapped):
                ...
                if _now - _last_edit >= 3 or (checked + skipped) >= total:
                    ...
                    progress = _elve_gold_progress(
                        title="Braintree Live Checking",
                        done=checked + skipped, total=total,
                        charged=0, approved=approved, declined=declined,
                        errors=0, elapsed=0,
                        extra=(
...
# ════════════════════════════════════════════════════════════════════════════════
#  /gate — Add / test custom gateway site
# ════════════════════════════════════════════════════════════════════════════════
...
async def cmd_gate(message: types.Message):
    ...
    args = message.text.split(maxsplit=1)
    if len(args) < 2 or not args[1].strip():
        # Show all gate sites for all types
        all_sites = get_all_user_gatesites(user_id)
        usage = (
            f"{pe(E['warn'])} {bold('Usage:')}\n\n"
            f"{pe(E['next'])} /gate {bold('gate_type url')}\n\n"
            f"{pe(E['bolt'])} {bold('Gate Types:')}\n"
            f"{pe(E['next'])} {bold('stripe')} — Stripe / Stripe Checkout\n"
            f"{pe(E['next'])} {bold('razorpay')} — Razorpay\n"
            f"{pe(E['next'])} {bold('braintree')} — Braintree\n"
            f"{pe(E['next'])} {bold('shopify')} — Shopify\n"
            f"{pe(E['next'])} {bold('woocommerce')} — WooCommerce\n"
            f"{pe(E['next'])} {bold('adyen')} — Adyen\n"
            f"{pe(E['next'])} {bold('paypal')} — PayPal\n"
            f"{pe(E['next'])} {bold('square')} — Square\n"
            f"{pe(E['next'])} {bold('authorize')} — Authorize.net\n"
            f"{pe(E['next'])} {bold('other')} — Other/Unknown\n\n"
            f"{pe(E['star'])} {bold('Auto-detect:')} /gate {bold('url')} {bold('(detects type from URL)')}\n\n"
            f"{pe(E['globe'])} {bold('Max')} {bold(str(GATESITE_MAX))} {bold('sites per gate type.')}\n"
            f"{pe(E['cross'])} {bold('Remove:')} /gaterem {bold('gate_type N')} {bold('or /gaterem gate_type all')}\n"
            f"{pe(E['link'])} {bold('View:')} /gatemysite {bold('[gate_type]')}"
        )
        if all_sites:
            for gate_type, sites in all_sites.items():
                numbered = "\n".join(f"{pe(E['next'])} {bold(str(i+1)+'.')} {bold(s)}" for i, s in enumerate(sites))
                usage += f"\n\n{pe(E['check'])} {bold(f'{gate_type.title()} Sites ({len(sites)}/25):')}\n{numbered}"
        await message.reply(usage)
        return

    # Parse gate_type and url
    parts = args[1].strip().split(maxsplit=1)
    if len(parts) == 1:
        # Auto-detect gate type from URL
        url = parts[0].strip()
        gate_type = detect_gate_type(url)
        await message.reply(
            f"{pe(E['loading'])} {bold('Auto-detecting gate type...')}\n\n"
            f"{pe(E['globe'])} {bold('URL:')} {bold(url)}\n"
            f"{pe(E['gem'])} {bold('Detected:')} {bold(gate_type)}"
        )
    else:
        gate_type = parts[0].strip().lower()
        url = parts[1].strip()

    if gate_type not in GATE_TYPES:
        await message.reply(
            f"{pe(E['cross'])} {bold('Invalid gate type!')}\n\n"
            f"{pe(E['bolt'])} {bold('Valid types:')}\n"
            f"{pe(E['next'])} {bold(', '.join(GATE_TYPES))}"
        )
        return
    ...
    if len(sites) >= GATESITE_MAX:
        await message.reply(
            f"{pe(E['cross'])} {bold(f'{gate_type.title()} site limit reached! ({GATESITE_MAX}/{GATESITE_MAX})')}\n\n"
            f"{pe(E['next'])} {bold('Remove one first:')} /gaterem {gate_type} {bold('N')}"
        )
        return
    if url in sites:
        await message.reply(
            f"{pe(E['warn'])} {bold('Site already in your list!')}\n\n"
            f"{pe(E['globe'])} {bold(url)}"
        )
        return
    ...
    loading_msg = await message.reply(
        f"{pe(E['loading'])} {bold('Testing site...')}\n\n"
        f"{pe(E['gem'])} {bold('Gate Type:')} {bold(gate_type.title())}\n"
        f"{pe(E['globe'])} {bold('URL:')} {bold(url)}\n"
        f"{pe(E['hourglass'])} {bold('Waiting for response...')}"
    )

    try:
        test_cc = "4242424242424242|12|2030|123"
        cc_parts = test_cc.split("|")
        cc, mm, yy, cvv = cc_parts[0], cc_parts[1], cc_parts[2], cc_parts[3]
        result = await _test_gate_site(gate_type, url, cc, mm, yy, cvv, proxy_list)
    except Exception as e:
        await safe_edit(loading_msg, 
            f"{pe(E['cross'])} {bold('Site test failed!')}\n\n"
            f"{pe(E['warn'])} {bold(str(e)[:100])}"
    ...
    if result.get("success"):
        add_user_gatesite(user_id, gate_type, url)
        new_count = len(get_user_gatesites(user_id, gate_type))
        await safe_edit(loading_msg, 
            f"{pe(E['check'])} {bold('Site added successfully!')}\n\n"
            f"{pe(E['gem'])} {bold('Gate Type:')} {bold(gate_type.title())}\n"
            f"{pe(E['globe'])} {bold('URL:')} {bold(url)}\n"
            f"{pe(R['gate'])} {bold('Response:')} {bold(result.get('msg', '')[:100])}\n"
            f"{pe(E['star'])} {bold(f'Total {gate_type} sites: {new_count}/25')}\n\n"
            f"{pe(E['bolt'])} {bold(f'You can now use /{gate_type} /m{gate_type} /{gate_type}txt')}"
        )
    else:
        await safe_edit(loading_msg, 
            f"{pe(E['cross'])} {bold('Site not working!')}\n\n"
            f"{pe(E['gem'])} {bold('Gate Type:')} {bold(gate_type.title())}\n"
            f"{pe(E['globe'])} {bold('URL:')} {bold(url)}\n"
            f"{pe(E['warn'])} {bold('Response:')} {bold(result.get('msg', '')[:100])}\n\n"
            f"{pe(E['next'])} {bold('Site must return a valid card response to be added.')}"
        )


async def _test_gate_site(gate_type: str, url: str, cc: str, mm: str, yy: str, cvv: str, proxy_list: list) -> dict:
    try:
        if gate_type == "stripe":
            if st1_gate:
                result = await asyncio.get_running_loop().run_in_executor(
                    CHECKER_POOL, lambda: st1_gate.check(cc, mm, yy, cvv, url, proxy_list=proxy_list)
                )
                return {"success": True, "msg": str(result)[:200]}
            return {"success": False, "msg": "Stripe gate module not loaded"}
        
        elif gate_type == "razorpay":
            status, msg, code, dbg = await _rz_execute(f"{cc}|{mm}|{yy}|{cvv}", url, proxy_list)
            return {"success": code != "timeout" and code != "exception", "msg": msg}
        
        elif gate_type == "braintree":
            if br_gate:
                result = await asyncio.get_running_loop().run_in_executor(
                    CHECKER_POOL, lambda: br_gate.check(cc, mm, yy, cvv, url, proxy_list=proxy_list)
                )
                return {"success": True, "msg": str(result)[:200]}
            return {"success": False, "msg": "Braintree gate module not loaded"}
        ...
        elif gate_type == "woocommerce":
            if st:
                result = await asyncio.get_running_loop().run_in_executor(
                    CHECKER_POOL, lambda: st.VW(f"{cc}|{mm}|{yy}|{cvv}", url, proxy_list=proxy_list)
                )
                return {"success": True, "msg": str(result)[:200]}
            return {"success": False, "msg": "WooCommerce gate module not loaded"}
        
        elif gate_type == "adyen":
            return {"success": False, "msg": "Adyen gate module not implemented yet"}
        
        elif gate_type == "paypal":
            return {"success": False, "msg": "PayPal gate module not implemented yet"}
        
        elif gate_type == "square":
            return {"success": False, "msg": "Square gate module not implemented yet"}
        
        elif gate_type == "authorize":
            return {"success": False, "msg": "Authorize.net gate module not implemented yet"}
        
        else:
            return {"success": False, "msg": "Unknown gate type"}
...
# ════════════════════════════════════════════════════════════════════════════════
#  /gaterem — Remove gate site(s)
# ════════════════════════════════════════════════════════════════════════════════

@router.message(Command("gaterem"))
async def cmd_gaterem(message: types.Message):
    joined = await check_user_joined(message.from_user.id)
    if not joined:
        await message.reply(JOIN_MSG, reply_markup=join_keyboard())
        return
    ...
    if len(args) < 2:
        all_sites = get_all_user_gatesites(user_id)
        if not all_sites:
            await message.reply(f"{pe(E['warn'])} {bold('No gate sites saved! Use /gate to add one.')}")
            return
        
        usage = f"{pe(E['warn'])} {bold('Usage:')}\n\n"
        usage += f"{pe(E['next'])} /gaterem {bold('gate_type N')} — {bold('remove site #N')}\n"
        usage += f"{pe(E['next'])} /gaterem {bold('gate_type all')} — {bold('remove all sites for gate_type')}\n\n"
        for gate_type, sites in all_sites.items():
            numbered = "\n".join(f"{pe(E['next'])} {bold(str(i+1)+'.')} {bold(s)}" for i, s in enumerate(sites))
            usage += f"\n{pe(E['check'])} {bold(f'{gate_type.title()} Sites ({len(sites)}/25):')}\n{numbered}"
        await message.reply(usage)
    ...
    gate_type = args[1].strip().lower()
    arg = args[2].strip() if len(args) > 2 else ""
    ...
    sites = get_user_gatesites(user_id, gate_type)
    if not sites:
        await message.reply(f"{pe(E['warn'])} {bold(f'No {gate_type} sites saved!')}")
        return

    if not arg:
        numbered = "\n".join(f"{pe(E['next'])} {bold(str(i+1)+'.')} {bold(s)}" for i, s in enumerate(sites))
        await message.reply(
            f"{pe(E['warn'])} {bold(f'Usage: /gaterem {gate_type} N')} or {bold(f'/gaterem {gate_type} all')}\n\n"
            f"{pe(E['check'])} {bold(f'Your {gate_type} sites ({len(sites)}/25):')}\n{numbered}"
        )
        return

    if arg.lower() == "all":
        del_user_gatesite(user_id, gate_type)
        await message.reply(f"{pe(E['check'])} {bold(f'All {gate_type} sites removed!')}")
        return

    try:
        idx = int(arg)
    except ValueError:
        await message.reply(f"{pe(E['warn'])} {bold('Use a number: /gaterem {gate_type} 1')} or {bold(f'/gaterem {gate_type} all')}")
        return

    if idx < 1 or idx > len(sites):
        await message.reply(f"{pe(E['warn'])} {bold(f'Invalid number. You have {len(sites)} {gate_type} site(s).')}")
        return

    removed = sites[idx - 1]
    del_user_gatesite(user_id, gate_type, idx)
    await message.reply(
        f"{pe(E['check'])} {bold('Gate site removed!')}\n\n"
        f"{pe(E['gem'])} {bold('Type:')} {bold(gate_type.title())}\n"
        f"{pe(E['globe'])} {bold(removed)}"
    )


# ════════════════════════════════════════════════════════════════════════════════
#  /gatemysite — View saved gate sites
# ════════════════════════════════════════════════════════════════════════════════

@router.message(Command("gatemysite"))
async def cmd_gatemysite(message: types.Message):
    joined = await check_user_joined(message.from_user.id)
    if not joined:
        await message.reply(JOIN_MSG, reply_markup=join_keyboard())
        return
    ...
    if not all_sites:
        await message.reply(
            f"{pe(E['warn'])} {bold('No gate sites saved!')}\n\n"
            f"{pe(E['next'])} {bold('Use /gate gate_type url to add one.')}"
        )
        return

    lines = [f"{pe(E['check'])} {bold('Your Gate Sites:')}\n"]
    
    for gate_type, sites in all_sites.items():
        numbered = "\n".join(f"{pe(E['next'])} {bold(str(i+1)+'.')} {bold(s)}" for i, s in enumerate(sites))
        lines.append(f"{pe(E['check'])} {bold(f'{gate_type.title()} Sites ({len(sites)}/{GATESITE_MAX}):')}")
        lines.append(numbered)
        lines.append("")

    lines.append(f"{pe(E['bolt'])} {bold('Commands:')} /gate /gaterem /gatemysite")
    lines.append(f"{pe(E['star'])} {bold('Use specific gate commands for checking:')} /stripe /razorpay /braintree /shopify /woocommerce")
